...
export default function Login({ onLogin, cancelLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Success! We save the token to the browser's local memory
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4 relative">
      {/* Background Video (Optional, matches your Manager view) */}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              type="email"
              required
              className="w-full bg-slate-800/50 border border-slate-600 text-white px-4 py-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all placeholder-slate-500"
              placeholder="agent@triage.com"
              value={email}
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-bold py-3.5 px-4 rounded-xl transition-all shadow-lg shadow-blue-500/30"
          >
